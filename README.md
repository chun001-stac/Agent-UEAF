...
| 实施规范 | 规范性（实现细化） | Machine Schema、Port/Event、持久化、测试和 Reference Implementation Profile |
| 参考架构 | 参考性 | 端到端时序、部署拓扑、存储、演化闭环和实施路径 |
| ADR | 规范性 | 已接受的关键架构决策及重审条件 |

发生冲突时，优先级依次为：**核心规范、总体设计、ADR、功能模块、实施规范、参考架构**。V2/V3 文档的 Future/Research 内容不覆盖 V1 Current 范围。
...
- [V1 文档一致性审计报告](docs/05-实施规范/06-V1文档一致性审计报告.md)

Codex/工程实现进入仓库后还必须先读取根目录 [AGENTS.md](AGENTS.md)。实施规范只负责把上层 Current V1 语义机器化；不得反向修改核心对象、状态、Port/Event、Authority 或 V1/V2/V3 边界。
...
Module 02..08 domain facts
  → TelemetryPort.EmitTrace / EmitMetric / EmitLog / EmitAudit
...
50. 所有跨模块持久化 Canonical Object 的 Machine Schema 必须组合/继承 `meta: ContractMeta`；领域文档只列增量字段时不得被解释为可以省略公共 `meta`。
51. 公共领域事件只有一套 `EventEnvelope`，线级字段与事件名以核心跨模块事件规范为唯一来源；Implementation/Reference 文档不得重新定义简化 Envelope 或 PascalCase Event Type。
52. `RuntimeAdapter`、`ContextBuildPort`、`ModelStepPort`、`ToolIntentPort`、`HandoffPort` 与 `TelemetryPort` 的公共接口以核心端口规范为唯一最小 SPI；功能/实施文档只能定义 optional/private convenience，不得建立第二套必需 Port。
53. API/跨进程失败统一使用 `ProblemDetail`，Port 失败统一使用 `PortResult<T>/PortError`；旧 `ErrorEnvelope` 只可作为迁移术语，不得进入新公共 Schema/API/Event。
54. Evolution Candidate Build 不得跳过 `GenomeManifest`：`MutationProposal -> machine validation -> GenomeManifest candidate -> Module 10 ReleaseCandidate` 是 V1 固定构建顺序。
