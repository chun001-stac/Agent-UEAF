...
V2 每个进入生态级 Finalist 的 Candidate 应同时评估：
...
V2 同时保留 Diversity / Monoculture Risk 约束：可保留替代 Species、不同模型/实现路线和若干 Elite Variants，限制共享依赖集中度。

为了控制评测成本，V2 MUST 采用 Funnel Evaluation：先执行便宜 Local Eval 和回归筛选，只对少量 Finalist 执行 Ecosystem Fitness 与 Diversity 检查。

V1 不要求实现独立 Ecosystem Fitness 或 DiversityPolicy；直接共享资源成本、fan-out、延迟和回归仅作为现有 Eval 维度处理。

## 后果

- V2 可避免成本和风险从局部 Agent 转嫁给生态；
- 降低共享隐藏缺陷导致的系统性故障；
- 评测与选择成本上升，因此必须使用漏斗式评测；
- 需要生态级资源归因和依赖关系视图；
- V1 保持轻量，不为该能力预建独立服务。
