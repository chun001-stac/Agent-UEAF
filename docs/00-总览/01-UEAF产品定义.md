...
6. **演化面**：V1 把结构化生产证据转化为受预算、评测、职责分离和发布治理约束的 Candidate；V2/V3 再扩展到生态共同进化与 Meta Evolution。
...
| 自我改进容易无限消耗 Token | 事件驱动、Sparse Mutation、有界 Working Set、既有 Budget Domain 和停止条件 |
...
- 事件驱动、Sparse Mutation、Evaluator Funnel、有界 Active Working Set 和明确停止条件。
...
| SRE | 容量、告警、故障响应、发布、恢复、灾备及演化成本/SLO |
| 质量治理团队 | EvalCase、评分器校准、QualityGateDecision、保留集和豁免管理 |
| Evolution Owner | Trigger 阈值、Mutation Strategy、权限 Profile、停止条件和 Candidate 提交治理 |
...
Experience/Lesson、Fitness、Lineage 在 V1 可作为内部记录或 Projection，不要求成为新的 Canonical Object。
...
12. 没有有效 Trigger 时不持续消耗强模型 Token 进行自我反思。
13. 能确定性评测的维度优先使用确定性/统计 Evaluator。
14. 历史可以增长，但 Active Working Set 必须有界。
15. 搜索算法可替换；GA 不是 Evolution 的定义。
16. Governance Kernel 是递归链外的稳定参照。
17. **Minimum Semantic Surface**：能用既有对象 + ref / Projection / metadata 表达时，不新增 Canonical Object。
18. V1/V2/V3 分离当前实现和未来设计；未来能力不得反向拖重当前实现。
...
- EvolutionRun 可以因平台期、预算耗尽、无有效候选、安全阻断或人工停止而终止；
