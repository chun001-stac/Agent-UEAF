...
### 3.3 Evolution V1 五个 Canonical Object
...
### 4.2 Evolution Objective Profile
...
任何校验失败 MUST 在 Candidate Build 前失败。

## 6. ID、Ref 与 Version 规范
...
6. 禁止恢复 `AgentGenome`、`CandidateRelease`、`EvolutionBudget` 等已被收敛的旧名。
...
- Subject/Object/Strategy Profiles 可校验；
- Mutation `changes[]` 可拒绝越权/越界修改；
- 示例 payload 全部自动验证；
- Schema compatibility 测试进入 CI；
- Codex 不需要通过 Markdown 自行猜字段类型、nullability 和枚举。
