...
> 实施一致性说明：本文中的对象 YAML 仅列领域字段；所有跨模块持久化对象仍继承核心对象规范定义的 `meta: ContractMeta`。`ReleaseManifest` 的版本字段以核心对象模型的“版本集合”语义为准，本文不得收窄成另一套单组件清单。
...
若某 Profile 只允许单 Agent/单 Prompt，机器 Schema MAY 对对应数组施加 `maxItems=1`，但不得把线级字段改名为单数，从而破坏 ReleaseManifest 作为兼容版本集合的通用语义。
