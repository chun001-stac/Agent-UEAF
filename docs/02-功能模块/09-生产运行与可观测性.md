...
sequenceDiagram
    ...
    alt edge rejected
        ...
        P->>S: CreateOrReuseTaskAndRun(phase=queued)
        S-->>P: task_id、run_id、revision
        P-->>G: queued response
        G-->>C: task_id、run_id
        S->>Q: Outbox 发布 RunQueued
        ...
        alt admitted
            ...
            R->>R: 校验 RuntimeStartCommand 并映射 RuntimeStartRequest
            R->>A: StartRun(RuntimeStartRequest)
            A-->>R: 规范 RuntimeEvent
            opt 有副作用动作
                R->>T: ToolIntent + tenant/task/run/release refs
                T->>T: 规范化、计算 action_key、Policy/Approval、预占 ActionRecord、取得动作 fencing
