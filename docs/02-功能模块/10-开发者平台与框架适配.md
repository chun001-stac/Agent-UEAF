...
| Adapter Development Kit | `RuntimeAdapter` SPI、事件桥和参考适配器 | Adapter 工件 |
...
ValidateRelease(release_subset, contract_ranges) -> ValidationResult
...
StreamEvents(RuntimeSession) -> RuntimeEventStream
Snapshot(RuntimeSession) -> RuntimeCheckpointRef | Unsupported
InspectProviderState(provider_state_ref) -> ProviderStateStatus
Close(RuntimeSession) -> CloseResult
```

`RuntimeCapabilities` 至少声明：同步/流式/异步、暂停恢复、Checkpoint、人工等待、工具调用拦截、Agent 交接、取消、结构化输出、原生 Provider State、事件顺序保证和最大版本范围。未支持的必需能力必须在准入前显式拒绝，不能运行中静默降级。

### 6.2 `RuntimeStartRequest`
...
`RuntimeStartRequest` 至少包含 `tenant_id`、`task_id`、`run_id`、`principal_context_ref`、`release_id`、解析后的 Agent/Prompt/Schema/Adapter 版本、可选初始 `ContextManifest`、预算、允许的能力引用和关联标识，并内含受限 `RuntimeExecutionContext`。该上下文提供超时/取消句柄，以及 `ContextBuildPort`、`ModelStepPort`、`ToolIntentPort`、`HandoffPort` 与 `TelemetryPort` 五个能力句柄，以支持多轮执行。Adapter 只能读取请求授予的引用，不能自行检索租户默认配置或“最新” Prompt，也不得持有模型长期凭据或绕过模块 03 调用模型。

### 6.3 事件规范化

框架原生事件通过 Adapter 映射为版本化 `RuntimeEvent`，再由 UEAF Runtime Shell 封装为 `EventEnvelope`。事件必须区分模型请求/结果、工具意图、交接意图、状态投影、Checkpoint 建议、流式片段、等待、错误和完成候选。

`EventEnvelope` 保留 `event_id`、`event_name`、`event_version`、发生/记录时间、租户、聚合类型/标识/版本、序号、生产者、关联/因果/Trace、主体、`release_id`、载荷 Schema、分类、用途和完整性引用。Adapter 不得伪造已经通过授权、动作成功、审计完成或发布批准。

### 6.4 状态与工具边界

- 框架原生状态可保存为 `CheckpointProjection` 或 opaque `provider_state_ref`；`TaskState` 与 `RunRecord` 仍由 UEAF 状态服务拥有。
...
- 远程 Agent 只有在被登记为有界、工具式请求/响应能力时才能经 `ToolIntentPort` 暴露；一旦涉及目标 Agent 接管、委派、子任务生命周期或独立预算，必须经模块 06 的 `HandoffPort`，并执行子任务准入、身份/权限重验和预算预留。

## 7. 框架接入边界

下表中的“原生概念”只用于说明适配位置，不构成 UEAF 的跨模块契约；具体 API 差异由对应 Adapter 版本处理。可复用层留在框架 Adapter 内，UEAF 保留层始终由 UEAF 服务持有语义所有权。
...
| 类型 | 作用 | 示例输出 | 不得承担 |
|---|---|---|---|
| Runtime Adapter | 桥接 Agent Loop 生命周期、状态投影和原生事件 | RuntimeEvent、CheckpointProjection | 企业授权、发布和持久 Task/Run 所有权 |
...
|---|---|---|
| Adapter 启动失败 | 运行不进入 `running`；按清单有界重试或停止 | 返回规范错误和版本信息 |
...
3. 重复、乱序和丢失事件可检测，`EventEnvelope` 的关联与因果链完整。
...
9. 结构化输出失败、模型限流、工具拒绝和框架异常映射为规范错误类别。
10. 不支持的 Profile 能力在构建或准入阶段显式失败。

## 12. 验收条件

1. 同一 `AgentDefinition` 能通过至少两个 Runtime Adapter 完成等价的只读案例，跨模块对象和审计语义不变。
2. LangGraph、MAF、OpenAI Agents SDK、Google ADK、CrewAI 均有明确能力声明、版本范围、状态映射和已知缺口。
